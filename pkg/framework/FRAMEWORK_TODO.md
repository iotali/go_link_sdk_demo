...
### 1. 修复服务调用路由机制 ⚠️
**问题**: 服务调用收到参数但未触发注册的handler
- 位置: `pkg/framework/framework.go:229-235`
- 现象: 日志显示 `params:{time:3}` 但返回 "Service handled by framework"
- 原因: `handleServiceCall` 函数中服务查找逻辑有误
- 修复: 正确匹配服务名称并调用对应handler
...
### 3. 添加事件上报功能
**缺失**: 框架只有属性上报，缺少事件机制
- 实现: `ReportEvent(eventName string, data map[string]interface{})`
- 示例: 过热告警、定时器完成事件
